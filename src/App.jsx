...
const LikeButton = (props) => (
    ...
    <div className={"like-button flex justify-center items-center content-center "+(props.didLike && 'didLike')}>
...
const PostCard = (item) => (
    <div className="post-card max-w-[1024px] mt-4 mx-auto card">
        ...
        <div className="post-interactions flex justify-between m-4">
            <div className="likes flex"><img className={'mr-3'} src={GreenLikeIcon} alt={"likes icon"}/>{item.likes} Likes</div>
            <div className="comments">{item.comments} Comments</div>
        </div>
        <div className="spacer"></div>
        <div className="controllers">
            <LikeButton didLike={item.didLike}/>
            <CommentButton/>
        </div>
    </div>
);
...
function App() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`api/hw/feed.json`);
                const data = await response.json();

                // וידוא שהמבנה data.data תקין לפני עדכון ה-state
                if (data && data.data) {
